...
class Policy(BasePolicy):
    ...
    def __init__(self):
        ...
        self._action_space = gym.spaces.Box(low=np.array([-1e10, -1e10, -np.pi, 0]), high=np.array([1e10, 1e10, np.pi, 1e10]), dtype=np.float32)
    ...
    def get_next_goal_pos(self, agent_obs):
        ...
        if goal_speed > speed_limit:
            next_goal_pos = current_pos + speed_limit * goal_dir * time_delta

        else: 
            next_goal_pos = closest_wp
